...
int main(int argc, char** argv) {
    ...
    bool status;    
    int nparts;
    double error = 0.01; //Set error variable
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
		...
		for(int i=0;i<nparts;i++) //Check each part against the defective part and once there is a match set the name of the current part.
		{
			geometry_msgs:: Pose pose = orphan_models_wrt_world[i].pose; //Tempor4ary variable to make code more readible 
		...
		status = robotBehaviorInterface.pick_part_from_box(current_part); //Discard 'current part' which should be the defective part
    ...
    for(int i=0;i<nparts;i++) //Run through all the parts in the box and discard them
    {
