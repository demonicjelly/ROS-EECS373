...
/*/////////////////////////////////////////////////////////////////////////
	...
	double des_x, des_y = 0;

	std::cout << std::endl;
	std::cout << "Enter the desired x coordinate of the gear: ";
	std::cin >> des_x;
	std::cout << std::endl << "Enter the desired y coordinate of the gear: ";
	std::cin >> des_y;
*/
...
while(ros::ok())
{
	...
	double offset_x = des_x - g_perceived_object_pose.pose.position.x;
	double offset_y = des_y - g_perceived_object_pose.pose.position.y;
	...
	double offset = 0.065;
///////////////////////move it to the correct position (still with higher z)//////////////////

	if(g_perceived_object_pose.pose.position.x > des_x + 0.01)
	{
    	flange_origin << g_perceived_object_pose.pose.position.x + offset, g_perceived_object_pose.pose.position.y, 0.3; 
	}
	else if(g_perceived_object_pose.pose.position.x < des_x - 0.01)
	{
    	...
    	flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y+offset, 0.3; 
	}
	else if(g_perceived_object_pose.pose.position.y < des_y - 0.01){
		flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y-offset, 0.3; 
	}
...
//go to pose to touch top of gear part; same x and y, but lower z value for flange origin
	...
	if(g_perceived_object_pose.pose.position.x > des_x + 0.01)
	{
    	flange_origin << g_perceived_object_pose.pose.position.x+offset, g_perceived_object_pose.pose.position.y, 0.01; 
	}
	else if(g_perceived_object_pose.pose.position.x < des_x - 0.01)
	{
    	flange_origin << g_perceived_object_pose.pose.position.x-offset, g_perceived_object_pose.pose.position.y, 0.01; 
    	...
    	flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y+offset, 0.01; 
	}
	else if(g_perceived_object_pose.pose.position.y < des_y - 0.01){
		flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y-offset, 0.01; 
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(g_perceived_object_pose.pose.position.x > des_x + 0.01)
	{
    	flange_origin << g_perceived_object_pose.pose.position.x + offset_x, g_perceived_object_pose.pose.position.y, 0.01; 
	}
	else if(g_perceived_object_pose.pose.position.x < des_x - 0.01)
	{
    	flange_origin << g_perceived_object_pose.pose.position.x - offset_x, g_perceived_object_pose.pose.position.y, 0.01; 
    	...
    	flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + offset_y, 0.01; 
	}
	else if(g_perceived_object_pose.pose.position.y < des_y - 0.01){
		flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - offset_y, 0.01; 
	}
...
//run the three trajectories to push it along

//loop back through will go to next if statement as first should be fulfilled 
//if at end results in breaking while 





	offset_x = des_x - g_perceived_object_pose.pose.position.x;
	offset_y = des_y - g_perceived_object_pose.pose.position.y;
	if(offset_x < 0.01 && offset_x > -0.01){
		if(offset_y < 0.01 && offset_y > -0.01){
			break;
		}
	}
	else{
		ROS_INFO("Position not reached, trying again");
	}
